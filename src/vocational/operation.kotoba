(ns vocational.operation
  "OperationActor -- one school operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (VocEdOps-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Curriculum Safeguarding Governor (:govern) and the
  ...
  `:certification/finalize`/`:graduation/finalize` ALWAYS reach this
  node when the governor is clean -- see `vocational.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [vocational.vocedopsllm :as vocedopsllm]
...
(defn build
  ...
  `vocational.store/Store`).
  opts:
    :advisor      -- a `vocational.vocedopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (vocedopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; VocEdOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (vocedopsllm/-advise advisor store request)]
            {:proposal p :audit [(vocedopsllm/trace request p)]})))
